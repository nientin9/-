...
for times in range(2):
    for lucky_draw_link in lucky_draw_links:
        ...
        print("question-popup:", question_popup_element_exist)
        if question_popup_element_exist == True:
            ...
            for questions in range(answer_count):
                ...
                options = driver.find_element_by_id(question_element_id).find_elements_by_class_name("fuli-option") #options = list[0, 1, 2]
                print("options:", options) 
                data_answer = driver.find_element_by_id(question_element_id).find_element_by_tag_name('a').get_attribute('data-answer') #data_answer = 'str' 3
            ...
            # watch_ad
            time.sleep(3)
            watch_ad_element = driver.find_element_by_id("btn-buy")
            print("watch_ad:", watch_ad_element)
        ...
        try:
            ...
            if_watch_ad.click()
            print("if_watch_ad:", if_watch_ad)
        except TimeoutException:
            print("if_watch_ad: TimeoutException")
            pass

        # close_ad
        # print(len(driver.find_elements_by_tag_name('iframe')))
        iframe = driver.find_elements_by_tag_name('iframe')[-1] #python+selenium 自動化過程中遇到的元素不可見時間以及webelement不可見的處理方法(https://iter01.com/467737.html)
